...
const ProfileOrder = () => {
    ...
    async function returnorderno() {
        ...
        axios.post(`${BASE_URL}/getreturnorderno`, data)
            .then((res) => {
                setorderid(res.data)
            })
    }
    ...
    const handleSaveChanges = async () => {
        ...
        if (validateForm()) {
            try {
                await axios.post(`${BASE_URL}/add_return_order`, { products: selectedProductData, order_id: order_id[0], user_id: custdecryptedUserId(), reasonid: reasonid });
                returnorderno()
                getOrderData()
                setCart([])
                setSelectedProducts([])
                                    ...
                                    {order.map((item) => {
                                        ...
                                        return (
                                            <tr>
                                                ...
                                                <td>
                                                    {orderid.some((ele) => ele.orderid == item.id) ? <p>Processing..</p> : <button onClick={() => {
                                                        getcartdata(item.id)
                                                        setError({})
                                                    }} className='btn btn-danger btn-sm' >Return </button>}
                                                </td>
                                                <td>
                                                ...
                                                >
                                                    <DialogTitle sx={{ m: 0, p: 2, width: "600px" }} id="customized-dialog-title">
                                                        Modal title
                                                    </DialogTitle>
                                                    <IconButton
                                                        aria-label="close"
                                                    ...
                                                    <DialogContent dividers>
                                                        ...
                                                        <div class="">
                                                            ...
                                                            />
                                                            {error.reason && <p className='text-danger float-right ' style={{display : "block"}}>{error.reason}</p>}

                                                            {error.checkbox && <span className='text-danger float-right'>{error.checkbox}</span>}
                                                        </div>
                                                    </DialogContent>
                                                    <DialogActions>
                                                        <Button sx={{ color: "#000" }} autoFocus onClick={handleSaveChanges}>
                                                            Save changes
                                                        </Button>
                                                    </DialogActions>
                                                </BootstrapDialog>
