...
const Checkout = () => {
  ...
  async function get_shipping_rate() {
    if (validateForm()) {
      ...
      try {
        const response = await axios.post(`${BASE_URL}/get_shipping_rate`, data);
        const shippingRates = response.data.shippingRates;
        if (shippingRates && shippingRates.length > 0) {
          const allRates = shippingRates.flatMap(vendor => vendor.rates?.data || []);
          const sortedRates = allRates.sort((a, b) => a.total_charges - b.total_charges);
